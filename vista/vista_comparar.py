...
class VistaComparar(tk.Frame):
    """
    Frame que permite comparar dos autómatas computando secuencias de inputs de
    hasta la longitud introducida
    """
    ...
    def __init__(self, master, controlador):
        ...
        boton_atras = tk.Button(self, text='<',
        ...
        self.tabla = ttk.Treeview(frame_resultados,
                                  columns=('qfa1', 'qfa2', 'comp'),
                                  selectmode='none',
        ...
        self.label_conformidad = tk.Label(self,
                                          text='Autómatas relacionados: ')
        self.label_conformidad.grid(row=2, column=0, columnspan=2, padx=5,
                                    sticky='w')
    ...
    def mostrar_comparacion(self, comparacion):
        ...
        for k, v in comparacion.items():
            self.tabla.insert('', 'end', text=k, values=(
                f'{float(v[0]): .2f}', f'{float(v[1]): .2f}', v[2]))
            conforme &= v[2]
        self.label_conformidad.configure(
            text='Autómatas relacionados: ' + str(conforme))
